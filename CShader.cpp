...
CShader::CShader(const std::string& name, const std::string& vs, const std::string& fs) : CObject(name)
{
	clean();
...
CShader::CShader(const std::string& name) : CObject(name)
{
	clean();
}

void CShader::CreaShader(const std::string& vs, const std::string& fs)
{
...
void CShader::clean()
{
	m_err = NO_ERR;
	m_uiProgram = 0;
	m_sVertSource = nullptr;
	m_sFragSource = nullptr;
	m_uiVertexShader = 0;
	m_uiFragmentShader = 0;
	m_iCompileStatusVertexShader = 0;
	m_iCompileStatusFragmentShader = 0;
	m_iLinkStatusShaders = 0;
}
...
GLchar* CShader::GetBufferFromFile(const std::string& in_szFilename)
{
	std::ifstream file;
	int size;
	char *buffer;
...
void CShader::setUniformShader1f(const std::string& variable, GLfloat v1)
{
	GLint val = glGetUniformLocation(m_uiProgram, variable.c_str());
	glUniform1f(val, v1);
}

void CShader::setUniformShader1i(const std::string& variable, GLint v1)
{
	GLint val = glGetUniformLocation(m_uiProgram, variable.c_str());
	glUniform1i(val, v1);
}

void CShader::setUniformShader2fv(const std::string& variable, GLsizei count, const GLfloat *value)
{
	GLint val = glGetUniformLocation(m_uiProgram, variable.c_str());
	if (val == -1)
...
void CShader::setUniformShader3fv(const std::string& variable, GLsizei count, const GLfloat *value)
{
	GLint val = glGetUniformLocation(m_uiProgram, variable.c_str());
	glUniform3fv(val, count, &value[0]);
}

void CShader::setUniformShaderMatrix3fv(const std::string& variable, GLsizei count, GLboolean traspose, const GLfloat *value)
{
	GLint val = glGetUniformLocation(m_uiProgram, variable.c_str());
	glUniformMatrix3fv(val, count, traspose, &value[0]);
}

void CShader::setUniformShaderMatrix4fv(const std::string& variable, GLsizei count, GLboolean traspose, const GLfloat *value)
{
	GLint val = glGetUniformLocation(m_uiProgram, variable.c_str());
	glUniformMatrix4fv(val, count, traspose, &value[0]);
}

void CShader::setUniformShaderTexture2D(const std::string& variable, GLuint textureid)
{
	GLint val = glGetUniformLocation(m_uiProgram, variable.c_str());
...
void CShader::setUniformShaderTextureCubeMap(const std::string& variable, GLuint textureid)
{
	GLint val = glGetUniformLocation(m_uiProgram, variable.c_str());
	glUniform1i(val, textureid);
	glActiveTexture(GL_TEXTURE0 + textureid);
