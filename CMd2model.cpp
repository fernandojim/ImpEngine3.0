...
// Constructor
//----------------------------------------
CMd2model::CMd2model(const std::string& s_filename) : CRenderingComponent()
{
	m_iFrame = 0;
	m_iTexelBuffer = 0;
...
//----------------------------------------
bool CMd2model::loadMD2Model(const std::string& s_filename)
{
	std::unique_ptr<CFileDef> file(new CFileDef(s_filename, true));
	glm::vec3 v;
	glm::vec3 n;
	glm::vec2 t;
	GLuint id_buffer = 0;
	...
	if (file->getIsOpen())
	{
		...
		{
			MessageBOX("Loading MD2 Model error", s_filename.c_str());
		...
		//Read skins info
		file->m_Filedef.seekg(m_header.offset_skins, std::ios::beg);
		...
		m_idTexture = Engine::Managers::TextureManager::getSingleton().loadTexture("texturas\\Imp.jpg");

		//Read texture coordinates info
		file->m_Filedef.seekg(m_header.offset_st, std::ios::beg);
		file->m_Filedef.read((char*)&m_texcoords[0], sizeof(md2_texcoord_t) * m_header.num_st);

		//Read triangles info
		file->m_Filedef.seekg(m_header.offset_tris, std::ios::beg);
		file->m_Filedef.read((char*)&m_triangles[0], sizeof(md2_triangle_t) * m_header.num_tris);

		//And finally read frames info
		file->m_Filedef.seekg(m_header.offset_frames, std::ios::beg);
	...
	for (int j = 0;j<m_header.num_frames;j++)
	{
		...
		// vertex buffer
		//id_buffer = getVAO()->CreateArrayBuffer(static_cast<void*>(&getVAO()->m_Vertex), m_vertsPerFrame * sizeof(float) * 3, GL_STATIC_DRAW);
		fvn.vBuffer = id_buffer;
		//getVAO()->m_Vertex.clear();

		// Normal buffer
		//id_buffer = getVAO()->CreateArrayBuffer(&(getVAO()->m_Normal[0]), m_vertsPerFrame * sizeof(float) * 3, GL_STATIC_DRAW);
		fvn.nBuffer = id_buffer;
		//getVAO()->m_Normal.clear();

		frame_vnBuffer.push_back(fvn);
	}

	//Creates the buffer to Texel
	//m_iTexelBuffer = getVAO()->CreateArrayBuffer(&getVAO()->m_Texel,  m_vertsPerFrame * sizeof(float) * 2, GL_STATIC_DRAW);
	//getVAO()->m_Texel.clear();
...
void CMd2model::WalkTo(glm::vec2 newPosition)
{
	...
	//salida << "m_position=(" << getVAO()->m_position->x << "," << m_position->z << ") angle=" << m_angle << "\n";
}
