...
CCubemap::CCubemap(const std::string& file)
{
	...
	loadCubeMapData(file);
}

bool CCubemap::loadCubeMapData(const std::string& cubemap_name)
{
	std::unique_ptr<CFileDef> filedef = std::unique_ptr<CFileDef>(new CFileDef(cubemap_name));
...
GLuint CCubemap::getCubeMapTextureId()
{
